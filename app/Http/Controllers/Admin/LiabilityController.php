...
class LiabilityController extends Controller
{
    ...
    {
        if($request->ajax()){
            $liabilities = ChartOfAccount::whereIn('account_head',[ 'Liabilities'])->get();
            $transactions = Transaction::with('chartOfAccount')->where('table_type', 'Liabilities')->where('status', 1);
        ...
        $coa = ChartOfAccount::whereIn('account_head',['Liabilities'])->get();
        
        $accounts = ChartOfAccount::where('sub_account_head', 'Account Payable')->get(['account_name', 'id']);
        return view('admin.transactions.liabilities', compact('coa','accounts'));
    }

    public function dkLiability(Request $request)
    {
        if($request->ajax()){
            $liabilities = ChartOfAccount::whereIn('account_head',['Liabilities'])->get();
            $transactions = Transaction::with('chartOfAccount')->where('table_type', 'Liabilities')->where('office', 'dhaka')->where('status', 1);
    ...
    public function ksaLiability(Request $request)
    {
        if($request->ajax()){
            $transactions = Transaction::with('chartOfAccount')->where('table_type', 'Liabilities')->where('office', 'ksa')->where('status', 1);
        ...
        $accounts = ChartOfAccount::whereIn('account_head',[ 'Liabilities'])->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->document) {
            ...
            $image->move(public_path('images/liabilities'), $imageName);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->document) {
            $image_path = public_path('images/liabilities/' . $transaction->document);
