                    ...
                    <div class="card-header p-0 pt-1">
                        ...
                        <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                            ...
                            <li class="nav-item ml-auto px-2">
                                <button class="btn btn-xs btn-success " data-toggle="modal" data-target="#chartModal" data-purpose="0">+ Add New Equity</button>
                        ...
                        <div class="col-md-6">
                            <div class="form-group" id="chart_of_account_container">
                                ...
                                <select class="form-control select2" id="chart_of_account_id" name="chart_of_account_id">
                                    ...
                                    @foreach($coa as $equity)
                                    <option value="{{ $equity->id }}">{{ $equity->account_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    var customerTBL = $('#equityTBL').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: charturl,
        ...
        columns: [{
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    if (row.document) {
                        return `<a class="btn btn-success btn-xs" href="{{asset('images/equity')}}/${row.document}" target="_blank">View</a>`;
                    } else {
                        return '';
                    }
                }
    ...
    var dkurl = "{{URL::to('/admin/dk-equity')}}";
    var customerTBL = $('#dkequityTBL').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: dkurl,
    ...
    var ksaurl = "{{URL::to('/admin/ksa-equity')}}";
    var customerTBL = $('#ksaequityTBL').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: ksaurl,
